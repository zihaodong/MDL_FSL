...
def check_params(params, **kwargs):
    ...
    for key, val in list(kwargs.items()):
        #key_defined = (key in list(params.keys()))
        key_defined = (key in list(params.keys()))
        if val is None:
...
class VideoPlayer:
    def __init__(self, video_item, base_trans = None, frame_trans_dist = None, frame_noise_dist = None, step = 1, offset = 0, max_len = np.inf, flo_method = None):
        ...
        if step > 0:
            ...
            b = - 1    
        #self.img_ids = list(range(a, b, self.step))
        self.img_ids = list(range(a, b, self.step))
...
class PASCAL:
    def __init__(self, db_path, dataType):
        ...
        #self.name_id_map = dict(list(zip(classes, list(range(1, len(classes) + 1)))))
        #self.id_name_map = dict(list(zip(list(range(1, len(classes) + 1)), classes)))
        self.name_id_map = dict(list(zip(classes, list(range(1, len(classes) + 1)))))
        self.id_name_map = dict(list(zip(list(range(1, len(classes) + 1)), classes)))
        self.dataType = dataType
	...
	#print osp.join(self.db_path, self.dataType + '_' + str(read_mode) + '_anns.pkl') 
    ...
    def getItems(self, cats=[], areaRng=[], read_mode = PASCAL_READ_MODES.INSTANCE):
        if len(cats) == 0:
            #catIds = list(self.id_name_map.keys())
            catIds = list(self.id_name_map.keys())
        else:
	...
	#print anns
        ...
        if read_mode == PASCAL_READ_MODES.SEMANTIC_ALL:
            old_ids = catIds
            #new_ids = list(range(1, len(catIds) + 1))
            #ids_map = dict(list(zip(old_ids, new_ids)))
            new_ids = list(range(1, len(catIds) + 1))
            ids_map = dict(list(zip(old_ids, new_ids)))
        for i in range(len(anns)):
            ann = anns[i]
...
class DBPascalItem(DBImageItem):
    def __init__(self, name, img_path, mask_path, obj_ids, ids_map = None):
        ...
        if ids_map is None:
            #self.ids_map = dict(list(zip(obj_ids, np.ones(len(obj_ids)))))
            self.ids_map = dict(list(zip(obj_ids, np.ones(len(obj_ids)))))
        else:
