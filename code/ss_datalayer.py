...
class DBInterface():
    ...
    def next_pair(self):
		...
		#print('I work well!!!!'+str(second_index)+'             '+str(first_index))
    ...
    def _remove_small_objects(self, items):
	    ...
	    #print np.sum(mask)
   	    #print '((((((((((((())))))))))))))))'
	    #print util.change_coordinates(mask, 32.0, 0.0).sum()
...
class PairLoaderProcess(Process):
    def __init__(self, name, queue, db_interface, params):
        ...
        self.first_label_scale = 1.0 if 'first_label_scale' not in params else params['first_label_scale']
